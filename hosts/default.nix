{ config, lib, pkgs, ... }:
{
  ...
  options.modules.hosts.defaultGateway = lib.mkOption { type = lib.types.nullOr lib.types.str; default = null; };
  ...
  config = {
    ...
    ] ++ (if config.modules.hosts.isLaptop && config.modules.hosts.amdcpu then [ ryzenadj ] else [ ]);
